...
def cand_times_table(df: pd.DataFrame):
    d = __printable_accuracies(df)

    caption = """
\\begin{table}[!h]
\\centering
\\caption{Czasu działania algorytmu dla różnych wariantów algorytmu CAND. Pogrubioną czcionką zaznaczono najlepszy wynik uzyskany na danym zbiorze danych.}
% p10, p30, m6p30, m8p30, m10p30, najlepszy z sub1, sub2, sub3
\\begin{tabular}{ll|ccccc}
\\hline
\\multicolumn{2}{c|}{\\textbf{Zbiór danych}} & \\multicolumn{5}{c}{Czas wykonania [s]} \\\\
...
def __printable_accuracies(df: pd.DataFrame) -> dict:
    ...
    for dataset in df["Dataset"].unique():
        ...
        vals[dataset] = {}
        vals[dataset]["P10_M10"] = float(results[dataset]["cand"]["P10_M10"]["Time Overall"].values[0])
        vals[dataset]["P30_M30"] = float(results[dataset]["cand"]["P30_M30"]["Time Overall"].values[0])
        vals[dataset]["P30_M06"] = float(results[dataset]["cand"]["P30_M06"]["Time Overall"].values[0])
        vals[dataset]["P30_M08"] = float(results[dataset]["cand"]["P30_M08"]["Time Overall"].values[0])
        vals[dataset]["P30_M10"] = float(results[dataset]["cand"]["P30_M10"]["Time Overall"].values[0])
        ...
        for key in printables[dataset].keys():
            if vals[dataset][keys_to_sizes[key]] == min(vals[dataset].values()):
