...
class Model:
    ...
    def update_loss_statistics(self):
        for branch,lossList in self.lossList.items():
            ...
            if branch not in self.lossStatisticsList.keys():
                ...
                if mean + var < self.lossStatisticsList[branch]["mean"] + self.lossStatisticsList[branch]["var"]:
                    self.lossStatisticsList[branch]["mean"] = mean # todo nowe minimum policzone na podstawie dużego okna
                    self.lossStatisticsList[branch]["var"] = var

                self.lossStatisticsList[branch]["prev_mean"] = prev_mean # todo a to z okna ostatnich 50 (45)
        ...
        if self.activeBranch in self.lossStatisticsList.keys():
            ...
            lossWin = self.lossList[self.activeBranch][-5:] # todo ostatnie 5 funkcji straty dla activeBranch
            driftFlag = min([l - d for l in lossWin])
            if not self.driftAlert:
                if driftFlag > 0: # todo sprawdźmy dla najmniejszego z ostatnich 5
                    self.driftAlert = 1
                else:
                    lossWin = sorted(self.lossList[self.activeBranch][-self.splitLen:]) # todo jak nie zadziałało to sprawdźmy dla 45
                    mean = np.mean(lossWin[0:-5])
                    var = np.var(lossWin[0:-5])
                    if mean + var > d:
                        self.driftAlert = 1
            if self.driftAlert: # todo inkrementujmy licznik warningów dryfu
    ...
    def train_model(self, feature, label):
        ...
        if self.alertNum == self.splitLen: # todo jeśli warningów 50
