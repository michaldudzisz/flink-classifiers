...
def atnn_size_time_comparison_table(df: pd.DataFrame):
    d = __printable_accuracies(df)

    caption = """
\\begin{table}[H]
\\centering
\\caption{Czas wykonania ze względu na rozmiar warstwy ukrytej. Podczas eksperymentów szybkość uczenia ustawiono na wartość $\\eta=0,02$, a parametr regularyzacji $\\lambda=5000$.}
\\begin{tabular}{llccccc}
\\hline
\\multicolumn{1}{c}{\\textbf{Zbiór danych}} & \multicolumn{5}{c}{\\textbf{Czas [s]}} \\\\ 
...
\\hline
\\end{tabular}
\\label{tab:atnn_hidden_sizes_times}
\\end{table}
    """
...
def __printable_accuracies(df: pd.DataFrame) -> dict:
    ...
    for dataset in df["Dataset"].unique():
        ...
        vals[dataset]["_32"] = float(vals[dataset]["_32"]["Time Overall"].values[0])
        vals[dataset]["_64"] = float(vals[dataset]["_64"]["Time Overall"].values[0])
        vals[dataset]["128"] = float(vals[dataset]["128"]["Time Overall"].values[0])
        vals[dataset]["256"] = float(vals[dataset]["256"]["Time Overall"].values[0])
        vals[dataset]["512"] = float(vals[dataset]["512"]["Time Overall"].values[0])
