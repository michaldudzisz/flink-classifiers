...
class MNIST:
    ...
    def generate(self, output_path, concepts, concept_length, visualize=False):
        with open(self.dataset_path, 'r') as csv_file, open(output_path, 'w') as output_csv_file:
            ...
            for line_number, row in enumerate(reader):
                ...
                angle = self._rotation_angle(line_number, concepts)
    ...
    def _rotation_angle(self, line_number, concepts):
        angle = 0
        if concepts[0][0] <= line_number <= concepts[0][1]:
            angle = 0
        elif concepts[1][0] <= line_number <= concepts[1][1]:
            angle = 90
        elif concepts[2][0] <= line_number <= concepts[2][1]:
            angle = 180
        elif concepts[3][0] <= line_number <= concepts[3][1]:
            angle = 270
    ...
    def _save_to_visualize_if_applicable(self, visualize, images, image, label, percentage):
        ...
        if 0.20 <= percentage <= 0.21 and not self._image_two_saved:
            if label == desired_label:
                images.append(image)
                self._image_two_saved = True

        if 0.60 <= percentage <= 0.61 and not self._image_three_saved:
            if label == desired_label:
                images.append(image)
...
if __name__ == "__main__":
    ...
    dataset_file = f"./datasets/fashion_abr.csv"
    mnist.generate(
        dataset_file,
        concept_length=10_000,
        concepts=[[0, 10_000], [10_001, 20_000], [20_001, 30_000], [30_001, 40_000]],
        visualize=True
    )
    # encode classes
